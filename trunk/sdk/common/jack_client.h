...
{
        return static_cast<nuclear::jack_client*>(arg)->process_callback(nframes);
}

void
libjack_thread_init_callback(void* arg)
{
	static_cast<nuclear::jack_client*>(arg)->thread_init_callback();
}

void 
libjack_freewheel_callback(int starting, void* arg)
{
	static_cast<nuclear::jack_client*>(arg)->freewheel_callback(starting);
}

int
libjack_buffer_size_callback(jack_nframes_t nframes, void* arg)
{
	return static_cast<nuclear::jack_client*>(arg)->buffer_size_callback(nframes);
}

int 
libjack_sample_rate_callback(jack_nframes_t nframes, void* arg)
{
	return static_cast<nuclear::jack_client*>(arg)->sample_rate_callback(nframes);
}

int 
libjack_xrun_callback(void* arg)
{
	static_cast<nuclear::jack_client*>(arg)->xrun_callback();
}
